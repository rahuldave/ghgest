...
Run `git diff` to show current changes (read-only, safe with git-butler).

Run `git-butler branch list` to see which branches own which changes, so the review scope is clear.
