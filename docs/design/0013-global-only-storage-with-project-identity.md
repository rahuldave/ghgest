...
the repository, and treat `.gest/` files as an explicit import/export format
rather than a live storage mode.
...
- Keeping state inside each checkout creates multiple uncoordinated copies of
  the same project's data.
- Automatic synchronization between an internal store and `.gest/` files would
  reintroduce two sources of truth and require conflict semantics we do not
  want to own.

The decision from [discussion #28][Discussion #28] is to make the global store authoritative,
anchor projects with an explicit committed ID, and keep repository snapshots as
an explicit workflow for sharing or versioning state alongside code.
...
- **Project identity** — each repository carries a stable project ID in
  ...
  state.
- **No local live mode** — `.gest/` directories are no longer treated as a
  continuously synchronized storage backend.
- **Explicit snapshots** — `gest export` writes the current project state to
  `.gest/`; `gest import` reads an existing `.gest/` snapshot into the global
  store. The database remains the source of truth before and after either
  command.
...
- Repository snapshots remain possible without requiring live in-repo storage
...
- Users must run explicit import/export commands when they want repository files
  to reflect the database state
...
- The old local live storage path (`store::artifact`, `store::task`, etc.) is
  removed entirely
- `.gest/` output remains available through explicit export/import commands,
