//! Per-entity sync adapter for projects.
//!
//! Stub created in the foundation task; the read/write bodies land in the
//! `Per-entity sync: project.yaml` task in phase 2.
...
/// Import every project file under `gest_dir` into SQLite.
pub async fn read_all(_conn: &Connection, _project_id: &Id, _gest_dir: &Path) -> Result<(), Error> {
  Ok(())
}

/// Export every project in SQLite to its per-entity file under `gest_dir`.
