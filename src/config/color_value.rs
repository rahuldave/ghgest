...
/// A user-specified color override that can be deserialized from three formats:
///
/// 1. Hex string: `"#RRGGBB"` — sets foreground color
/// 2. Named color: `"red"`, `"bright cyan"`, etc. — sets foreground color
/// 3. Inline table: `{ fg = "...", bg = "...", bold = true, ... }` — full style control
...
impl ColorValue {
  /// Apply this color value as overrides onto an existing [`Style`].
...
impl Serialize for ColorValue {
    ...
    S: serde::Serializer,
  {
    ...
    // Simple case: fg-only with no modifiers
    if self.bg.is_none()
      && !self.bold
      && !self.dim
      && !self.italic
