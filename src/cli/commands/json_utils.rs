//! Shared JSON conversion utilities for CLI commands.

/// Convert a YAML metadata mapping to a JSON value.
pub fn format_metadata_for_json(mapping: &yaml_serde::Mapping) -> serde_json::Value {
...
/// Convert a single YAML value to its JSON equivalent.
pub fn yaml_value_to_json(v: &yaml_serde::Value) -> serde_json::Value {
  match v {
...
/// Serialize an [`crate::model::Artifact`] to a JSON value matching the `artifact show --json` shape.
pub fn artifact_to_json(artifact: &crate::model::Artifact) -> serde_json::Value {
  serde_json::json!({
...
mod tests {
  mod format_metadata_for_json {
    ...
    fn it_converts_string_values() {
      ...
      let json = crate::cli::commands::json_utils::format_metadata_for_json(&mapping);
